    ...
    def move_crate(self, instructions: list) -> None:
        ...
        if not self.stacks:
            ...
            for i in range(instructions[0]):
                lifted_crates.append(self.stacks[instructions[1]-1].pop())

            print(f"Moved crate(s) '{lifted_crates}' from {instructions[1]} to {instructions[2]}")
            for crate in reversed(lifted_crates):
                self.stacks[instructions[2]-1].add_crate(crate)
