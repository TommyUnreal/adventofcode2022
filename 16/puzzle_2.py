...
def traverse_graph(is_open:set, valves:dict, total_flow:int, current_valve:Valve, elephant_move:bool, time_remaining:int):
    ...
    Args:
        ...
        elephant_move (bool): True if elephant is moving, False if you are.
    ...
    if current_valve.name not in is_open:
        ...
        traverse_graph(is_open.union([current_valve.name]),valves, total_flow + current_valve.flow_rate * time_remaining, current_valve, elephant_move, time_remaining - 1)
        #Elephant is on the way
        if not elephant_move:
            traverse_graph(is_open.union([current_valve.name]),valves, total_flow + current_valve.flow_rate * time_remaining, valves["AA"], True, 26-1)
    else:
        for target in [valves[possible_target] for _,possible_target in current_valve.paths_rated if not valves[possible_target].name in is_open]:
            ...
            traverse_graph(is_open, valves, total_flow, target, elephant_move, time_remaining - time_cost)
...
if __name__ == "__main__":
    ...
    valves_to_search = {k:v for (k,v) in valves.items() if v.name in worth_valves}

    max_total_flow = 0
    #lower than 2355
    #lower than 2323
    traverse_graph(is_open, valves_to_search, 0, valves["AA"], False, 26-1)
